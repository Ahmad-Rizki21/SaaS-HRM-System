...
import { 
  ...
  XCircle, FileSpreadsheet, CheckSquare, Square, ClipboardList 
} from "lucide-react";
import * as XLSX from 'xlsx';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

export default function PermitReportsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Gagal ambil data perizinan", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    ...
    worksheet['!cols'] = [
      { wch: 5 }, { wch: 25 }, { wch: 20 }, { wch: 18 }, { wch: 18 }, 
      { wch: 40 }, { wch: 15 }, { wch: 30 }
    ];

    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Laporan Perizinan");
    XLSX.writeFile(workbook, `Laporan_Perizinan_${new Date().getTime()}.xlsx`);
  };
  ...
  const generatePDF = async () => {
    ...
    doc.text("LAPORAN REKAPITULASI PERIZINAN KARYAWAN", 45, 26);
    ...
    autoTable(doc, {
      startY: 50,
      head: [['NO', 'KARYAWAN', 'JENIS', 'PERIODE', 'STATUS']],
      body: tableData,
      headStyles: { 
        fillColor: [139, 0, 0], 
    ...
    doc.save(`Laporan_Perizinan_${new Date().getTime()}.pdf`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="dash-page-header">
        <div>
          <h1 className="dash-page-title">Riwayat & Laporan Perizinan</h1>
          <p className="dash-page-desc">Data komprehensif riwayat perizinan (Sakit, Izin, Keperluan Pribadi) seluruh karyawan.</p>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder="Cari karyawan, jenis izin, atau alasan..." 
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-20 text-center text-gray-400 font-bold">Tidak ada riwayat perizinan yang ditemukan.</td>
                  </tr>
                ...
                <tr key={item.id} className={`hover:bg-gray-50/50 transition-colors group ${selectedIds.includes(item.id) ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-xs font-black shadow-sm italic">
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="font-bold text-gray-900 text-lg">Detail Perizinan</h3>
              <button 
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-6">
                 ...
                 <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                    <div className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xl italic shadow-md">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">Tipe: {selectedItem.type} | Status: {selectedItem.status}</p>
                    </div>
                 </div>
                 ...
                 <div className="p-4 bg-gray-50 border border-gray-100 rounded-2xl">
                    <p className="text-[10px] uppercase font-black text-gray-400 mb-2">ALASAN / KETERANGAN</p>
                    <p className="text-sm text-gray-600 leading-relaxed italic font-medium">"{selectedItem.reason || '-'}"</p>
                 </div>

                 {selectedItem.signature && (
                    <div className="p-4 border rounded-2xl">
                        <p className="text-[10px] uppercase font-black text-gray-400 mb-2">TANDA TANGAN</p>
                        <div className="bg-gray-50 rounded-xl p-2 flex justify-center">
                            <img src={selectedItem.signature} alt="Signature" className="max-h-32 object-contain" />
                        </div>
                    </div>
                 )}
              </div>
