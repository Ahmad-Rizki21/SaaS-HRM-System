...
<head>
    <meta charset="utf-8">
    <title>Surat Permohonan Cuti - {{ $leave->user->name }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <p>Nomor: CR/{{ date('Y/m', strtotime($leave->created_at)) }}/{{ str_pad($leave->id, 4, '0', STR_PAD_LEFT) }}</p>
    </div>
    ...
    <div class="content">
        ...
        <table class="info-table">
            ...
            <tr>
                ...
                <td>:</td>
                <td>{{ $leave->user->role?->name ?? 'Karyawan' }}</td>
            </tr>
        </table>

        <p>Mengajukan permohonan cuti dengan rincian sebagai berikut:</p>
        <table class="info-table">
            <tr>
    ...
    <table class="footer-table">
        <tr>
            <td>
                ...
                <p><strong>({{ $leave->user->name }})</strong></p>
                <p style="font-size: 9px; margin-top: -10px;">{{ date('d-M-Y', strtotime($leave->created_at)) }}</p>
            </td>
            <td>
                <p>Mengetahui,</p>
                <div class="signature-box"></div>
